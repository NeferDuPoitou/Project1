...
// Allocates and returns a new list containing n
// nodes starting at node t in list l.
...
// iterates over the list l and applies f while cond returns
// a non 0 value, or until
// the end of l.
// returns early and sets errno to EINVAL if l, cond or f are NULL,
// or if l is an empty list
void		lst_do_while(t_lst *l, int (*cond)(t_lnode *), \
				void (*f)(t_lnode *));
...
void		vector_sort_range(t_vector *v, size_t start, \
			size_t end, int (*cmp)(const void *, const void *));;
void		vector_iter(t_vector *v, void (*f)(void *));
...
t_string	*string_substr(t_string *s, size_t start, size_t len, \
				int alloc_lvl);
